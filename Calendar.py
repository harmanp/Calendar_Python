...
"""
    ...
    if start_time <= end_time:
        if date in calendar:
            calendar.update({date: [start_time, end_time, title]})
            ...
            calendar[date] = [{"start": start_time, "end": end_time, "title": title}]
                #calendar[date] = [{start_time, end_time, title}]
            save_calendar(calendar)
            return True
        else:
            return False
...
def command_show(calendar):
    ...
    Desc_Dates = sorted(calendar, key=lambda x: (-calendar[x], x))
    all_events = '\n'

    for each_date in Desc_Dates:
...
"""
    ...
    if date in calendar.keys():
        if len(calendar[date]) == 0:
            del calendar[date]
            return True
        for x in range(len(calendar[date])):
            if calendar[date][x]['start'] == start_time:
                #remove ddel calendar[date][x]
                del calendar[date]
                return True
            else:
                return "There is no event with start time of " + \
            str(start_time) + " on date " + date + " in the calendar"
    else:
...
def load_calendar():
    ...
    for line in data_file:
        ...
        calendar[1] = raw_data[0]
        calendar[1][0]["start", "end", "title"] = raw_data[1]
    data_file.close()
    return calendar
...
def parse_command(line):
    ...
    final_arg = line.split(' ')
    if final_arg[0] == 'add':
        if len(final_arg) < 2:
            err_list.extend(['add DATE DETAILS'])
            return err_list
        else:
            events = ' '+",".join(final_arg[4:])
        if is_calendar_date(final_arg[1]):
            return final_arg[:4] + [events]
        else:
            err_list.extend(['not a valid calendar date'])
            return err_list
    elif final_arg[0] == 'show':
        if len(final_arg) != 1:
            err_list.extend(['show'])
            return err_list
        return final_arg
    elif final_arg[0] == 'delete':
        if len(final_arg) != 3:
            err_list.extend(['delete DATE NUMBER'])
            return err_list
        if not is_calendar_date(final_arg[1]):
            err_list.extend(['not a valid calendar date'])
            return err_list

        elif not is_natural_number(final_arg[2]):
            err_list.extend(['not a valid event index'])
            return err_list
        else:
            return final_arg
    elif final_arg[0] == 'quit':
        if len(final_arg) != 1:
            err_list.extend(['quit'])
            return err_list
        return final_arg

    elif final_arg[0] == 'help':
        if len(final_arg) != 1:
            err_list.extend(['help'])
            return err_list
        return final_arg
    else:
        return ['help']
